...
默认配置下，所有页面文件放在 `src/moduleName/pages` 文件夹下，默认的index页面在`src\moduleName\pages\index\index.jsx`  
...
如上所述，模块的页面文件位于 `pages` 文件夹下，在pages下新建一个文件夹 `mall`,在此文件夹下新建文件 `index.jsx`，内容如下
...
- 一个页面最外层都需要使用 `TopView` 组件，并且一个页面上只能存在一个 `TopView` 组件，这和路由的实现和全局弹窗有关系，请查看[页面](/docs/course/started/page)了解详情
- 路由方法都需要是用 `nav` 或者 `route`，两个是相同方法的不同表现形式，请查看[路由](/docs/course/started/route)了解详情
...
```
有关 `duxui` 的使用文档 请查看 [duxui](/docs/duxui/start)，duxui的所有示例代码，在上面提到，请安装 `duxuiExample` 模块查看
...
详情请查看[用户配置](/docs/course/started/config)
